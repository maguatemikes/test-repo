...
/** Proxy → crm-api /api/templates (GET list, POST create). */
...
export async function GET(req: Request) {
  ...
  const res = await fetch(`${API_BASE}/templates${url.search}`, { headers: cookie ? { cookie } : {}, cache: "no-store" });
...
export async function POST(req: Request) {
  ...
  const res = await fetch(`${API_BASE}/templates`, { method: "POST", headers: { "Content-Type": "application/json", ...(cookie ? { cookie } : {}) }, body });
